...
public static class Say
{
	...
	{
		if (n < 0 || n > 999_999_999_999L) throw new ArgumentOutOfRangeException();
		...
		for (int i = 0; i < labels.Count; i++)
		{
			...
			{
				var div = n/value;
				var _base = $"{(value > 10 ? InEnglish(div,false) : tens[(int)div])}{label}";
				var mod = n % value;
				if (mod == 0) return _base;
				return $"{_base}{separator}{InEnglish(mod, false)}";
			}
		}
		return string.Empty;
	}
