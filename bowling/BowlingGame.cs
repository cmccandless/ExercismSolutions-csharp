...
namespace Exercism.bowling
{
    public class BowlingGame
    {
        ...
        private bool MinFrames => Frames.Count >= 10;
        ...
        public int? Score() => MinFrames && (CheckSpare || CheckStrike || CheckNormal) ? Frames.First().Sum() : null;
        private class Frame
        {
            ...
            private int? value => Ball1 + (Ball2 ?? 0);
            private bool InvalidTotal => value > 10 || value < 0;
            private bool ValidNormal => IsNormal || Next == null || Number >= 10;
            private int? extendedValue => value + Next.Ball1 + (IsSpare ? 0 : Next.IsStrike ? Next.Next.Ball1 ?? 0 : Next.Ball2);
            public int? Value => InvalidTotal ? null : ValidNormal ? value : extendedValue;
            ...
            private string special => IsStrike ? ",Strike" : IsSpare ? ",Spare" : string.Empty;
            public override string ToString() => $"{Number}:{Value}({Ball1},{Ball2}{special})";
        }
    }
}